...
class TabModel(BaseEstimator):
    ...
    optimizer_params: Dict = field(default_factory=lambda: dict(learning_rate=2e-2))
    ...
    def __post_init__(self):
        ...
        self.device = paddle.set_device(define_device(self.device_name))
        if self.verbose != 0:
    ...
    def fit(
        ...
        for callback in self._callback_container.callbacks:
            if isinstance(callback, LRSchedulerCallback):
                self.optimizer_params.update({"learning_rate": callback.scheduler})
                break
        self._set_optimizer()
    ...
    def _train_batch(self, X, y):
        ...
        batch_logs["loss"] = loss.detach().numpy().item()
    ...
    def _set_optimizer(self):
        """Setup optimizer."""
        if self.clip_value:
            clip = paddle.nn.ClipGradByNorm(clip_norm=self.clip_value)
        else:
            clip = None
        self._optimizer = self.optimizer_fn(
            parameters=self.network.parameters(), grad_clip=clip, **self.optimizer_params
        )
    ...
    def _compute_feature_importances(self, loader):
        ...
        for data, targets in loader:
            data = paddle.cast(data, 'float32')
            M_explain, masks = self.network.forward_masks(data)
            feature_importances_ += M_explain.sum(axis=0).detach().numpy()
