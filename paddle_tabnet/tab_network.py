...
class GBN(nn.Layer):
    ...
    def forward(self, x):
        chunk_num = int(np.ceil(x.shape[0] / self.virtual_batch_size))
        sections = []
        for c in range(chunk_num):
            if c < chunk_num - 1:
                sections.append(self.virtual_batch_size)
            else:
                sections.append(x.shape[0] - (chunk_num - 1) * self.virtual_batch_size)
        chunks = paddle.split(x, sections, axis=0)
...
class TabNetEncoder(nn.Layer):
    def __init__(
        ...
        if self.n_shared > 0:
            shared_feat_transform = nn.LayerList()
            for i in range(self.n_shared):
                if i == 0:
                    shared_feat_transform.append(
                        Linear(self.input_dim, 2 * (n_d + n_a), bias_attr=False)
                    )
                else:
                    shared_feat_transform.append(
                        Linear(n_d + n_a, 2 * (n_d + n_a), bias_attr=False)
                    )
        ...
        self.feat_transformers = nn.LayerList()
        self.att_transformers = nn.LayerList()
    ...
    def forward(self, x, prior=None):
        ...
        for step in range(self.n_steps):
            ...
            M_loss += paddle.mean(
                paddle.sum(paddle.multiply(M, paddle.log(M + self.epsilon)), axis=1)
            )
            # update prior
            prior = paddle.multiply(self.gamma - M, prior)
            # output
            masked_x = paddle.multiply(M, x)
    ...
    def forward_masks(self, x):
        ...
        prior = paddle.ones(x.shape)
        M_explain = paddle.zeros(x.shape)
        att = self.initial_splitter(x)[:, self.n_d :]
        masks = {}
        ...
        for step in range(self.n_steps):
            ...
            d = ReLU()(out[:, : self.n_d])
            # explain
            step_importance = paddle.sum(d, axis=1)
            M_explain += paddle.multiply(M, paddle.unsqueeze(step_importance, axis=1))
            # update attention
            att = out[:, self.n_d :]
...
class TabNetPretraining(nn.Layer):
    def __init__(
        self,
        input_dim,
...
class TabNetNoEmbeddings(nn.Layer):
    def __init__(
        self,
        input_dim,
        output_dim,
        ...
        if self.is_multi_task:
            self.multi_task_mappings = nn.LayerList()
            for task_dim in output_dim:
                task_mapping = Linear(n_d, task_dim, bias_attr=False)
                initialize_non_glu(task_mapping, n_d, task_dim)
                self.multi_task_mappings.append(task_mapping)
        else:
            self.final_mapping = Linear(n_d, output_dim, bias_attr=False)
            initialize_non_glu(self.final_mapping, n_d, output_dim)

    def forward(self, x):
        res = 0
        steps_output, M_loss = self.encoder(x)
        res = paddle.sum(paddle.stack(steps_output, axis=0), axis=0)
...
class AttentiveTransformer(nn.Layer):
    def __init__(
        self,
        input_dim,
        output_dim,
        ...
        self.fc = Linear(input_dim, output_dim, bias_attr=False)
        ...
        if mask_type == "sparsemax":
            # Sparsemax
            self.selector = sparsemax.Sparsemax(axis=-1)
...
class FeatTransformer(nn.Layer):
    def __init__(
        self,
        input_dim,
        output_dim,
...
class GLU_Block(nn.Layer):
    ...
    def forward(self, x):
        scale = paddle.sqrt(paddle.to_tensor(np.array([0.5]).astype('float32')))
        ...
        for glu_id in layers_left:
            x = paddle.add(x, self.glu_layers[glu_id](x))
            x = x * scale
        return x


class GLU_Layer(nn.Layer):
    def __init__(
        ...
        if fc:
            ...
            self.fc = Linear(input_dim, 2 * output_dim, bias_attr=False)
    ...
    def forward(self, x):
        x = self.fc(x)
        x = self.bn(x)
        out = paddle.multiply(x[:, : self.output_dim], paddle.nn.functional.sigmoid(x[:, self.output_dim :]))
        return out


class EmbeddingGenerator(nn.Layer):
    """
    ...
    def __init__(self, input_dim, cat_dims, cat_idxs, cat_emb_dim):
        ...
        self.continuous_idx = paddle.ones(input_dim, dtype='bool')
    ...
    def forward(self, x):
        ...
        for feat_init_idx, is_continuous in enumerate(self.continuous_idx):
            ...
            if is_continuous:
                ...
                cat_feat_counter += 1
        # concat
        post_embeddings = paddle.concat(cols, axis=1)
        return post_embeddings


class RandomObfuscator(nn.Layer):
    """
...
if __name__ == '__main__':
    model = TabNet(input_dim=54, output_dim=20)
    export_weight_names(model)
    inputs = np.load("/Users/alex/Desktop/inputs.npy").astype('float32')
    # model_dict = paddle.load("/Users/alex/Desktop/paddle_tabnet.pdparams")
    from utils import load_pretrained_model
    load_pretrained_model(model, "/Users/alex/Desktop/paddle_tabnet.pdparams")
    out, loss = model(paddle.to_tensor(inputs))
    # loss.backward()
    print(out.numpy()[0,:10])