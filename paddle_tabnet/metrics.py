...
def UnsupervisedLoss(y_pred, embedded_x, obf_vars, eps=1e-9):
    ...
    reconstruction_errors = paddle.multiply(errors, obf_vars) ** 2
    batch_stds = paddle.std(embedded_x, axis=0) ** 2 + eps
    features_loss = paddle.matmul(reconstruction_errors, 1 / batch_stds)
    # compute the number of obfuscated variables to reconstruct
    nb_reconstructed_variables = paddle.sum(obf_vars, axis=1)
    ...
    loss = paddle.mean(features_loss)
    return loss
